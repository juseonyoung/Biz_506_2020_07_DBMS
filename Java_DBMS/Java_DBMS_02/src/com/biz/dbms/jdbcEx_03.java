...
public class jdbcEx_03 {
	...
	public static void main(String[] args) {
		...
		/*
		 * UPDATE 구현 1. 전체(검색조건에 맞는) 데이터 리스트를 보여주고 2. 내용을 변경할 데이터의 seq 값을 키보드에서 입력받고 3.
		 * 데이터를 1개 select하고 4. 각 칼럼데이터를 보여주고 5. 변경할 내용을 입력받는다. 6. 완료가 되면 update method를
		 * 호출하여 변경을 실시한다.
		 * 
		 */
		...
		try {
			...
			System.out.println("SEQ는 숫자만 가능합니다");
			return;
		}
		...
		// 변경할 주문 번호를 입력했을 때 (빈칸으로 두지 않았ㅇ들 때)
		// 입력한 주문번호를 orderVO.setnum에 세팅한다.
		// 그냥 엔터만 입력하면 통과되므로 원래 o_num에 있던 값은 유지된다.
		if (!str_num.isEmpty()) {
		...
		while (true) {
			System.out.printf("변경할 가격( %d,Enter:변경안함) >>", orderVO.getO_price());
