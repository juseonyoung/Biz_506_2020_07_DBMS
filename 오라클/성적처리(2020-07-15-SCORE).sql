...
-- 디폴트로 설정된 값을 지정한 것으로 이해하고 INSERT를 수행하라
...
----------------------------------
-- 학생별 성적의 총점과 평균을 계산하여 리스트로 확인하기 
...
SELECT AVG(sc_kor) AS 국어평균,
        ...
        AVG(sc_art)AS 미술평균 --빈칸 넣으면안됨
FROM tbl_score;

-- SELECTION 적용하여 구현해보기
-- 학번이 20001~20010까지인 학생의 리스트만 보고 싶을 때
SELECT * FROM tbl_score
WHERE sc_num BETWEEN '20001' AND '20010';


-- 학번이 20001~20010까지인 학생들만 추출하여 과목별 총점을 보여줌
SELECT SUM(sc_kor) AS 국어총점,
        SUM(sc_eng)AS 영어총점, 
        SUM(sc_math)AS 수학총점,
        SUM(sc_music)AS 음악총점,
...
-- 학번이 20001~20010까지인 학생들만 추출하여 과목별 평균을 보여줌
SELECT AVG(sc_kor) AS 국어평균,
        AVG(sc_eng)AS 영어평균, 
        AVG(sc_math)AS 수학평균,
        AVG(sc_music)AS 음악평균,
...
UNION ALL
-- SUM 함수 이용하여 각 과목별 총점 계산하면서 의미없는 (Dumy) 칼럼을
-- ==과목총점== 이라는 값으로 만들었다.
-- 통계,집계 함수를 사용할 때 통계,집계 함수로 묶이지 않은 칼럼(ex 과목총점)은
-- 무조건 group by 절에 칼럼을 나열해 주어야 한다!!!!

SELECT '==과목총점==', --의미없는 칼럼.. 반드시 group by로 나타내줘야
        SUM(sc_kor) AS 국어, --총점 보여주는
        SUM(sc_eng)AS 영어, 
        SUM(sc_math)AS 수학,
        SUM(sc_music)AS 음악,
        SUM(sc_art)AS 미술
